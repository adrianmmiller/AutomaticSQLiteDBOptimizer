...
# modded from here: https://stackoverflow.com/questions/32311768/bash-wait-until-cpu-usage-gets-below-a-threshold
  sleep 60
  threshold=$1
  while true; do
...
fucntion optimize () {
	for i in $(find /data/* -iname "*.db"); do
		...
		echo -ne " Database $i: VACUUM=$resVac REINDEX=$resIndex ANALYZE=$resAnlz" 2>&1 | ts '[%d/%m/%Y %H:%M:%S]' | tee -a $log_file
		fi
	done
}


# First run after uinstall
